...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			checkErr: func(err error) bool {
				e := &ValidationErrors{}
				return errors.As(err, e) && errors.Is(e, ErrValidateForUnexportedFields)
			},
		},
		{
			...
			checkErr: func(err error) bool {
				e := &ValidationErrors{}
				return errors.As(err, e) && errors.Is(e, ErrInvalidValidatorSyntax)
			},
		},
		{
		...
		},
		{
			...
			args: args{
				v: struct {
					SliceS []string `validate:"len:4"`
					SliceI []int `validate:"max:10;min:5"`
				}{
					SliceS: []string{"efgh","bcda"},
					SliceI: []int{6,7,8},
				},
			},
			wantErr: false,
		},
		{
			...
			args: args{
				v: struct {
					SliceS []string `validate:"len:5"`
					SliceI []int `validate:"max:5;min:1"`
					SliceIA []int `validate:"max:-"`
				}{
					SliceS: []string{"efgh","bcda"},
					SliceI: []int{6,7,8},
					SliceIA: []int{1,2,3},
